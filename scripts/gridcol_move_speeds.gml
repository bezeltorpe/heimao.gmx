...
// Y + X AXIS                                                                                   // The purpose of this is to adjust for correct angular movement, and save alot of collision checks - see IF,ELSE below
var angle=point_direction(0,0,vx,vy);                                                           // Determine total movement angle
var pvx=vx*abs(dcos(angle));                                                                    // adjust movement vector (x)
var pvy=vy*abs(dsin(angle));                                                                    // adjust movement vector (y)
if !gridcol_place_meeting(x+pvx,y+pvy) {x=round(x+pvx);y=round(y+pvy);}                         // IF there is no collision in combined direction of trglobal.avel, move there
else{                                                                                           // ELSE: seperate axis checks required
...
if vx!=0{                                                                                       // Do we need to move in X direction at all?
    ...
    repeat(abs(vx)){                                                                            // LOOP: repeat as often as vx:
        if !gridcol_place_meeting(x+svx,y) {x+=svx;}                                            // no collision in direction of trvel: move one pixel in that direction!
